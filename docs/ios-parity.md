...
**Next executable slice:** Capture the key Android playback reference in FND-02 and run FND-03's locked-device live-radio + real-FFT spike immediately, including DEC-07 HTTP fixtures; do not wait for a polished shell. Finish the wider source/effect matrix, FND-04 platform decisions and FND-05 fixtures before signing off phase 0. Then build VIS-01–06 and the radio playback loop; keep all remaining rows visible through full parity. FND-01 stays `R` until the pinned CI workflow runs green on GitHub.
